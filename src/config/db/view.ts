...
export const vwEventDetails = pgView("vw_event_details").as((qb) =>
  qb
    .select({
      ...
      rsvpDeadline: sql`${event.rsvpDeadline}`.as("rsvpDeadline"),
      ...
      respondedBy: sql`${invitation.respondedBy}`.as("respondedBy"),
      status: sql`${invitation.status}`.as("status"),
      respondedAt: sql`${invitation.respondedAt}`.as("respondedAt"),
      invFamilyId: sql`${invitation.familyId}`.as("invFamilyId"),
      category: sql`${invitation.category}`.as("category"),
      invitationName: sql`${invitation.invitationName}`.as("invitationName"),
